...
{
	final static String field_id = "roleid";
	final static String field_name = "rolename";
	final static String field_description = "description";

	final static String baseURL = "http://localhost:8080/monolith/rext/rolemgmt/roles";

	// Echte Role-ID von der ersten Rolle aus GET-Abruf
	static String realRoleID;
	...
	void getRoles()
	{
		...
		RoleResourceTest.realRoleID = jsonUser.getString(field_id);
	}
	...
	{
		// act
		invocationBuilder = target.path("/{id}").resolveTemplate("id", RoleResourceTest.realRoleID).request(MediaType.APPLICATION_JSON);
		final Response res = invocationBuilder.options();
	...
	{
		// act
		invocationBuilder = target.path("/search").queryParam("name", "SysAdm").request(MediaType.APPLICATION_JSON);
		final Response res = invocationBuilder.get();
